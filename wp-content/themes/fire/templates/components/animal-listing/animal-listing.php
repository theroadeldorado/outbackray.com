...
<?php $section->start(); ?>
  <div>
    <div class="fire-container">
      ...
      if ($animals_query->have_posts()) : $index = 0; $color_index = 0; ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5" x-data="animalGallery" @keydown.escape="closeGallery()" @keydown.arrow-left="prevImage()" @keydown.arrow-right="nextImage()">
          <?php while ($animals_query->have_posts()) : $animals_query->the_post();
            ...
            <div>
              ...
              <div class="fixed inset-0 z-[1001] grid-stack">
                ...
                <?php endforeach; ?>
                <div class="fixed bottom-4 left-4 rounded-lg bg-green bg-texture-leaves max-w-[500px] pl-8 pr-4 pt-4 pb-4">
                  ...
                  <div class="flex justify-between gap-4">
                    ...
                    <div class="flex justify-end self-end gap-3 shrink-0">
                      ...
                      <button @click="prevImage()" class="text-white font-bold flex items-center justify-center size-8 p-2 bg-emerald-900 hover:bg-emerald-800 ease-in-out duration-300 transition-all hover:scale-105 rounded-full rotate-90">
                        <?php new Fire_SVG('icon--chevron-down'); ?>
                      </button>
                      <button @click="nextImage()" class="text-white font-bold flex items-center justify-center size-8 p-2 bg-emerald-900 hover:bg-emerald-800 ease-in-out duration-300 transition-all hover:scale-105 rounded-full -rotate-90">
                        <?php new Fire_SVG('icon--chevron-down'); ?>
                      </button>
                    </div>
                  </div>
